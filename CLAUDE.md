...
## 视觉调俯仰：方向已定，方案在 `lz/doc/VISION-GIMBAL-PITCH.md` `[V]`（2026-09-25）

**方向**（用户 2026-09-25 定）：**用视觉取图识别，把「云台俯仰角」从
「按几何公式算」改成「看画面把杆中点对到画面中央」。** 几何公式保留，
但**降级为初始值**。

**动机**：公式 `-atan2(飞机相对目标高度 - h/2, 半径)` 吃三个输入，其中
**`h`（杆高）是写死的 15.0**（`lz_pole_source.c:347` 与 `:389`）。
那两处的注释写着"绕飞不需要精确值"—— **那句话在俯仰写死常数的时代成立，
现在公式直接用 `h`，不成立了**（15→20 m 在默认剖面下差 0.3°、12.5 m 外
6.5 cm，确实小；但真正的问题是**公式假设的一切只要有一处不对，相机就偏了
而画面上看不出异常** —— 上一轮写死的 −15° 差 65° 就是同一形状）。

**核心认识**：公式里的 `-h/2` 就是「瞄杆中点」，与视觉瞄准**是同一个物理点**，
两者可互相校验也可互相替代。⇒ **绕飞时高度定 + 半径定 ⇒ 俯仰恒定**
（用户指出）⇒ **「测一次」与「持续测」几何等价**，飞行前一次性测定是
**充分的**，不是妥协。它顺带避开了「`GimbalManager_Rotate` 与航线里
`gimbalRotate` 抢同一个云台」这个未实测的冲突。

**三个环节**：① 取图（`lz/app/lz_vision_source.c`）② 量测（`lz_vision`）
③ 应用（换掉剖面里的 `gimbalPitchDeg`，**③ 不需要新代码**）。

⚠️ **②有一个真实缺口**：要"杆的中点"就需要杆自身的上下端，而
`lz_pole_extent()` 算出来的 `poleTop/poleBottom` **在 `lz_vision.c:610-611`
被 `(void)` 丢掉了**，且**从未被验证过**（6 张照片的黄金值里只有杆列 x）。
**不能用旗面框的中点替代** —— 现场是"旗在杆顶"，一根 15 m 的杆旗面占顶上
1.5 m，**旗中点比杆中点高 6.75 m**（12.5 m 外 = 28°）。那是"瞄错点"，
不是"误差大"。

### 更正：「PSDK 不能转自带云台」不成立 `[X]`

旧记录（HANDOFF 与 API-MAP）写着不能 —— **那是把两个模块混为一谈**：

| 头文件 | 服务对象 | setter |
|---|---|---|
| `dji_gimbal.h` | **第三方**云台（SkyPort / X-Port） | ❌ 只有 `Init`/`DeInit`/`RegCommonHandler` |
| `dji_gimbal_manager.h` | **挂载在飞机上的**云台（含自带） | ✅ **9 个**，含 `Rotate(pitch,roll,yaw,time)` 单位 deg |

四条支持证据（妙算3 高级功能清单含「云台管理」`[V]`；能力矩阵
`Gimbal Management`=advanced 且 M4 系列高级功能需妙算3；
**issue #563** 有人在 M4T+妙算上用它控 pitch；issue #555 能设模式）
见方案文档 §12。

⚠️ **#555 说的限制是 `yaw`**（与本文「靠机头 towardPOI、不靠云台偏航」
一致，**不冲突**）；⚠️ **#563 说 `Rotate` 实速只有下发的 10–20%** ⇒
用它必须**设置 + 读回确认**（`GIMBAL_ANGLES` 走回调缓存，getter 必崩）。

### 硬约束速查（详见方案文档 §6）

- **`pixFmt` 选错不是报错，是花屏** ⇒ 取图侧对**收到的**格式做校验并计数
- **`buf` 在回调返回后失效**（从官方样例反推，非文档所写）⇒ 必须当场拷贝
- **回调跑在 SDK 线程** ⇒ 拷贝在锁内、检测在锁外（与控件回调同一条纪律）
- **PSDK 没有 FOV / 内参 getter**（已 grep 全部 39 个头文件）
  ⇒ 只能用规格值或上机标定
- **82° 是 DFOV（对角），不是水平 FOV** —— 当成水平用会错 **11°**。
  正确算式：`tan(VFOV/2) = tan(DFOV/2) · H/√(W²+H²)`（4:3 + 82° ⇒ **55.1°**）
- **M4T 有三个镜头：广角 82° / 中长焦 35° / 长焦 15°**，而
  `E_DjiLiveViewCameraSource` 只给 `M4T_VIS=1 / IR=2 / 4K=3` ——
  **对不上**。取到哪个镜头是**头号待定项**（换算分母差 5 倍），由探针答
- **`DjiLiveview_StartImageStream` 标注 only on DJI manifold3** ——
  我们正是妙算3

### 本轮已交付 / 未交付

**已交付**：方案文档 + `lz_vision_source.{h,c}`（取图骨架，能编译）+
`lz_liveview_probe.c`（**上机验证取图的探针**，`scan` 模式逐个试
source 并存 PPM 回本机认镜头）。

**未交付**：步骤 2–6（垂直 FOV 纯函数、杆端点、像素→角度、云台设+读回、
接进流程）；主链路**未接入取图**（测定还没做，接了没有调用点）。
