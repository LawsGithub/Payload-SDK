...
但本项目的转弯模式是 `toPointAndStopWithDiscontinuityCurvature`（**到点停**），
n 个点就是 n 次起停。改动这一节时注意：**改上限的理由如果变了，
`lz_plan.h` 里的注释也要跟着改。**

**航点多了会暴露一个既有现象**：生成 n 个点、飞机飞 n−1 段，
覆盖 `(n-1)/n` 圈。8 点覆盖 315°、16 点覆盖 337.5° —— 越多越接近闭合，
但**永远不闭合**（见 `lz_plan.c` 的「关于闭合」）。这是既有设计，不是缺陷。
