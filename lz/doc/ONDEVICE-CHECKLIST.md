...
**可控范围 −90°~70°**（2026-09-26 更正，见 `lz_plan.h` 的常量。
早期记录的 +30° 是错的 —— 那是照抄 M4E；DJI 官网 M4T 是 70°，
固件说明里也有「M4T gimbal now supports up to 70° upward tilt」）。

现场对照表（杆高按 15 m 算 → 瞄中点 7.5 m）：
...
| 20 m | 80 m | −74.6° | ✅ |
| 20 m | 20 m | −32.0° | ✅ |
| 12.5 m | 比杆**低** 10 m | +54.5° | ✅（上仰） |
| 12.5 m | 比杆**低** 30 m | +71.6° | ❌ 超 70° |
| 5 m | 比杆**低** 20 m | +79.7° | ❌ 超 70° |

⚠️ **最后三行是要现场记住的**：飞机比杆低的时候需要**上仰**，
而上仰能力只有 70° —— **飞机低得越多、半径越小，越可能够不着**。
够不着时 `LzPlan_Validate` 会在起飞前用 `LZ_ERR_RANGE` 拒掉
（这是刻意的：把"物理上做不到"变成起飞前的明确错误，而不是上机后
相机纹丝不动却不知道原因）。

⚠️ **判据**：绕飞时机头对杆（`towardPOI`），杆应**大致停在画面中央**。
若杆明显偏在画面上缘或下缘，说明俯仰算错了（或目标高填错了）——
...
## 3.9 取图探针 —— 视觉调俯仰的头号待定项（**最便宜的先验项之一**）

方案见 [`VISION-GIMBAL-PITCH.md`](VISION-GIMBAL-PITCH.md)。这一步不碰飞控、
不碰航点，**只是把图像流打开看看**，成本比控件 5 还低。

```bash
./lz_liveview_probe scan        # 依次试 source = 0,1,2,3，各跑 4 秒并存一帧
# 存的帧在当前目录 lz_frame_1_<src>.ppm（P6，拷回本机直接能看）
```

**要回答的问题（VISION-GIMBAL-PITCH.md §9 #1，卡着换算的分母）：**

- [ ] **四路里哪几路真能收到帧？** 每行打印 `帧数 / fps / 丢帧计数`。
      一帧都收不到 → 见探针输出里的三条可能（source 不存在 / 飞机不在飞行界面 /
      需要先注册编码回调）。
- [ ] **收到的是哪个镜头？** 把 PPM 拷回本机看视场：
      最宽 → 广角 82°；中等 → 中长焦 35°；最窄 → 长焦 15°。
      灰度无彩色 → 红外（**不该用它做红色检测**）。
- [ ] **通道顺序是 RGB 还是 BGR？** 红旗在 PPM 里显示为**蓝色** ⇒ 是 BGR，
      要在 `LzFrame.isBgr` 标出来。**这一项错了，红色检测会彻底失效**
      （而表现是"检测不到目标"，病因却在线序上）。
- [ ] **真实分辨率与宽高比？** 用于垂直 FOV 的换算
      （`tan(VFOV/2) = tan(82°/2) · H/√(W²+H²)`，4:3 时是 55.1°）。
- [ ] **帧率稳不稳、丢帧多不多？** 决定"等画面稳定"要等多久（几帧）。

**结论写到哪**：`doc/VISION-GIMBAL-PITCH.md` §9 #1 划掉，并同步
`lz_vision_source.c` 的 `LzVisionSource_DefaultConfig()` —— 那里的
`position` / `source` 现在**都是猜的**（`M4T_VIS` 是不是广角，无从判断）。

⚠️ **顺带一个不用上机就能答的判据**（§9 #2，优先级高）：
激光照射时看 `T_DjiCameraManagerLaserRangingInfo.screenX/screenY`
（单位 **0.1%** 屏幕坐标），与图像流里检测到的杆列 `pixel.u` 对照 ——
**一致就说明激光与图像流同轴**，`LzPole_RecordLaser()` 与视觉瞄准
指向同一个画面中心。这一步用 `lz_rangefinder_probe 1 -1` 连续监视就能读。
