                    ...
                    int seconds)
{
    ...
    if (st != LZ_OK) {
        printf("  开流失败：%s\n", LzStatus_Str(st));
        printf("  （`DjiLiveview_Init` 要在 `DjiCore_Init` 之后；"
...
static void print_assumptions(void)
{
    ...
    printf("默认配置（**未经上机验证，是当前最优猜测**）：\n");
    printf("  position = %d\n", (int)d.position);
    printf("  source   = %d\n", (int)d.source);
    printf("  pixFmt   = %d (PIXFMT_RGB_PACKED=%d)\n", (int)d.pixFmt, (int)PIXFMT_RGB_PACKED);
    printf("  自述     = %s\n", LzVisionSource_DescribeAssumptions());
}

int main(int argc, char **argv)
{
    printf("[lz_liveview_probe] 探针：M4T 图像流取图\n");
    ...
    printf("初始化 PSDK（阻塞 2-4 秒，需飞机通电并连接）...\n");
    ...
    if (scan) {
        ...
        printf("\n扫描完毕：4 路里 %d 路取到了帧。\n", gotCount);
        printf("把取到帧的那几个 PPM 拷回本机，按下面的判据认镜头：\n");
        printf("  · 视场最宽、能看到大范围地面   → 广角 82°（等效 24mm）\n");
        printf("  · 视场中等、目标被放大         → 中长焦 35°（等效 70mm）\n");
        printf("  · 视场最窄、细节最大           → 长焦 15°（等效 168mm）\n");
        printf("  · 灰度、无彩色                 → 红外（不该用这一路做红色检测）\n");
        printf("  · 红旗在 PPM 里显示为蓝色      → 通道是 BGR，要在 LzFrame.isBgr 标出来\n");
        printf("结论记进 doc/VISION-GIMBAL-PITCH.md §9 #1，并同步 lz_vision_source.c 的默认值。\n");
    } else {
        /* 默认组合；也可显式给三个参数 */
        const LzVisionSourceConfig d = LzVisionSource_DefaultConfig();
