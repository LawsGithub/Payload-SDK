...
/** @name 云台俯仰角的可控范围
 *
 * ⚠️ **这里是"拒掉物理上做不到的指令"的判据，不是"描述相机规格"。**
 * 四个量互不相同，别互相替代：
 *
 * | 量 | M4T 取值 | 出处 | 用途 |
 * |---|---|---|---|
 * | 机械限位 Tilt | −140° ~ **113°** | 规格页 | 硬件能到的极限 |
 * | 软限位 Tilt | −90° ~ 35° | 规格页 | **未开扩展时**的软件限制 |
 * | **可控转动范围 Tilt** | **−90° ~ 70°** | 规格页 | **本条用它** |
 *
 * ⚠️ **`+70°` 与早期记录的 `+30°` 不一致，早期那个是错的**（2026-09-26 更正）：
 * 网上多份 M4T 规格转载页写 `Tilt: -90° to 35°`（照抄 M4E），而
 * **DJI 官网 M4T 页面写的是 `俯仰：-90° 至 70°`**（M4E 仍是 35°）。
 * 佐证：DJI 企业版固件说明里有「M4T gimbal now supports up to 70° upward
 * tilt」，且**仅列 M4T**。⇒ 取 70。
 *
 * ⚠️ **为什么不取 35°（软限位）**：软限位是"没开扩展时"的值，而
 * `DjiGimbalManager_SetPitchRangeExtensionEnabled()` 就是把它切成扩展值的。
 * WPML 那条路是否自动开扩展**未实测** ⇒ 取 35 会**误拒**本可执行的指令
 * （如飞机低于杆时的上仰），而取 70 最多让飞机自己拒（那会报错，不是静默）。
 * **误拒 = 该做的做不了且看不见；误放 = 做不了的会报错。后者可观测。**
 *
 * ⚠️ **M4E 是 35°**。换机型时这两个数要跟着改。
 *
 * ⚠️ **WPML 规范没给 `gimbalRotate` 的 `gimbalPitchRotateAngle` 取值域**
 * （`40.common-element.md` 只写"不同云台可转动范围不同"），所以没有
 * 协议侧的权威值可引 —— 只能按机型规格 + 上机实测。
 */
/** @{ */
...
/**
 * @brief 按几何反算云台俯仰角：让**目标中点**落在画面中心所需的俯仰
 *
 * ## 为什么必须是反算，而不是让调用方填一个常数
 *
 * 绕飞时飞机始终停在距目标 `radiusM` 的圆周上，所以**视线俯角随风力以外的
 * 一切固定**（高度定、半径定 ⇒ 俯角定，飞行中不需要变）—— 这个直觉是对的。
 * 但它的**值**随半径与高度变化，写死一个数必然在某些组合下错得离谱：
 *
 *   俯仰 = -atan2(飞机相对高度 - 目标高, 半径)
 *
 * | 半径 | 高度 | 看向杆底 | 看向杆顶 |
 * |---|---|---|---|
 ...
 *
 * 实测（2026-09-24）：写死的 **-15°** 只在"低高度 + 大半径"下碰巧对，
 * 在默认滑杆值（半径 12.5 m、高度 80.9 m）下偏差 **66°** —— 相机根本
 * 没对着目标，而屏幕上看起来一切正常。
 *
 * ## 为什么瞄"目标中点"而不是杆顶或杆底
 *
 * 目标是竖直的杆，`targetHeightM` 是它的顶。取中点 = 让整根杆落在画面
 * 中央附近，上下都留余量 —— 与 `LzGeo_BearingDeg` 让机头瞄杆心同一个理由
 * （水平方向瞄圆心、垂直方向瞄半高）。
 *
 * ⚠️ **目标高度未知时（`heightM <= 0`）按 0 处理**，即瞄准地面高度那一层。
 * 不猜一个"典型杆高"：猜错会让相机偏离，而偏了多少无从判断。
 *
 * @param radiusM        环绕半径 m，必须 > 0
 * @param altAboveTargetM 飞机相对**目标底部**的高度 m（= 相对起飞点高度 + 起飞点与目标的椭球高差）
 * @param targetHeightM  目标高 m；<= 0 表示未知，按 0 处理
 * @return 俯仰角，单位度，向下为负
 *
 * ⚠️ **本函数不钳位，越界由 `LzPlan_Validate` 拒绝。**
 *
 * 早先这里钳到相机限位，但那样会把"飞机在目标头顶正上方"（俯仰 −90.4°）
 * 悄悄改成 −90°，并让下面这条真实缺陷永远看不出来：
 * **飞机低于目标时该往上看，而相机上仰能力有限，超出就是物理上做不到。**
 * 钳位把"做不到"伪装成"做得到"，与"静默的失败等于假装成功"是同一个形状。
 * 改为照实返回，让 `LzPlan_Validate` 用 `LZ_ERR_RANGE` 明确拒绝。
 *
 * ⚠️ **本函数对"目标在飞机上方/下方"是对称处理的**：它只回答"要让中点
 * 居中该指向哪"，不问那个指向可不可达（用户 2026-09-26 指出的要点 ——
 * 无论红旗在飞机上面还是下面，只要在广角画面里且云台够得着，就该转过去
 * 让屏幕中心落在旗上）。所以两种情形都返回带符号的角（飞机在目标上方
 * → 负 = 下压；在下方 → 正 = 上仰），可达性由上面那组限位常量 +
 * `LzPlan_Validate` 判定。 */
double LzPlan_ComputeGimbalPitchDeg(double radiusM, double altAboveTargetM, double targetHeightM);
...
typedef struct {
    ...
    double gimbalPitchDeg;  /*!< 拍摄时云台俯仰，向下为负。
                                 *
                                 * ⚠️ **当 `autoGimbalPitch` 为 true 时本字段被忽略** ——
                                 *    俯仰角改由几何反算，见下方那一节。 */
    bool autoGimbalPitch;   /*!< true = 按几何反算俯仰角，忽略 `gimbalPitchDeg` */
