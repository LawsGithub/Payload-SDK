...
- 锚点: `feature/liangzhourenwu` @ `e5b79e14a59f9b63acd07a8315d3aa45cc159df6`（2026-09-26 14:05）
- 漂移检查: `git rev-parse HEAD~1` 应 = `e5b79e14a59f9b63acd07a8315d3aa45cc159df6`——HEAD 是本次 handoff 提交，
  其 parent 才是本快照记录的 SHA；变了说明快照可能过期
- **远端已同步到 `2645061`** `[V]`（2026-09-26 推送成功）。
  其上还有 `e5b79e1`（测试未初始化读取修复）与本次 handoff 提交**未推送**
  —— 推送**必须走代理**，直连会挂起（见 §5）
- **设备走 USB 扩展坞可达** `[V]`（2026-09-26 本次实测）：**`192.168.1.180`**
  （设备 `eth0` 直接落在本机 `192.168.1.0/24`，本机经 `eth3`）。
  ⚠️ **地址变了** —— 旧记录的 `192.168.42.120` 是 USB 网络共享那条路
  （本机 `eth4`），本次 `eth4` **不存在**。两条路都通，别假设是哪一个：
  先 `ip -4 -br addr` 看本机哪个接口有线，再 `ip neigh` 看 ARP。
  凭据不变（`dji`/`dji`）。
- **设备时钟落后本机 43.7 小时** `[V]`（设备 2026-09-24 17:3x vs 本机 09-26 13:xx）。
  ⇒ 打包的 `--mtime` **必须从设备取时间**再往前减 60 秒，
  按本机时间算出来的是**未来**。见 `CLAUDE.md`「设备侧打包清单」。
- **飞机本次全程关机**，所以只做到"编译 + 链接 + 跑桌面测试"，
  取图探针**卡在 `DjiCore_Init`**（预期行为，不是缺陷）
...
### 本轮（第三段，2026-09-26）新增 `[V]`

**设备走通了 USB 扩展坞这条路**，并在其上完成了 **aarch64 本机编译 + 链接自检**：

```text
设备地址 192.168.1.180（eth0 直连本机网段），凭据不变 dji/dji
设备时钟落后本机 43.7 小时 ⇒ 打包 --mtime 必须从设备取时间
设备侧编译（cmake LZ_BUILD_PSDK_APP=ON）：我们代码 0 warning
产物 4 个：lz_app / lz_liveview_probe / lz_mission_probe / lz_rangefinder_probe
GLIBC 要求 2.17（设备 2.31）✅   ldd 全部解析成功
设备上跑 aarch64 桌面测试：ctest 7/7（逐目标 rc 全 0，检查数
  geo34 / plan1144 / bridge12 / kmz22 / wpml140 / validate173 / pole53）
⚠️ 飞机全程关机 ⇒ 探针卡在 DjiCore_Init（预期），取图探针**未取得真帧**
```

**顺带修掉一条真实的 UB** `[V]`：`lz_test_wpml.c` 的 `seg[0]` 在段数为 0 时
被读（只有设备上那条 `-Wmaybe-uninitialized` 能看出来的），已加
`LZ_CHECK(ns > 0)` 并包进 `if`。
⚠️ **反向验证诚实记录**：拆掉守卫后桌面 gcc 11 / -O2 **既不报警也不变红**
（UB 不保证可观测）—— 所以这条的守门机制是**设备上那条编译器警告**，
不是测试失败。

**同时更正了一处规格错**：云台俯仰上限 **30° → 70°**（早期那个是照抄 M4E 的）。
详见 §3 与 `CLAUDE.md`「云台俯仰的可控范围是 −90°~70°」。

用户 2026-09-26 的指示（改变了本文件的排序）：
1. 「红旗顶的实际高度决定俯仰」**是错误说法** ⇒ 无论旗在飞机上面还是下面，
   只要在广角画面里、在云台范围内，就该让**屏幕中心落在旗上**。
2. 改用 **USB 扩展坞**连接妙算（`192.168.1.180`），设备编译不再是最急的事。
...
第三段（2026-09-26，俯仰限位更正 + 设备编译）:
  桌面 stub 7/7、hsv 8/8；lz_test_plan **1144 项检查 0 失败**（新增限位用例）
  反向验证：把 LZ_GIMBAL_PITCH_MAX_DEG 从 70 改回 30 ⇒ lz_test_plan rc=1
            （那条用例先报值不等，再连带边界断言失败）
  **设备侧 aarch64 本机编译**：我们代码 0 warning（仅 lz_test_wpml.c 一条
            pre-existing 的 -Wmaybe-uninitialized，非本轮引入）
  设备侧 ctest 7/7；逐目标 rc=0（geo34/plan1144/bridge12/kmz22/wpml139/
            validate173/pole53）
  ⚠️ 飞机未通电 ⇒ 取图探针卡在 DjiCore_Init 满 40 s 超时，**未取到真帧**
...
第三段切片（`a8613c3`..已推送；其后两条**尚未推送**）：
  a8613c3 fix(lz): 云台俯仰上限 30°→70°，并把限位收敛到头文件唯一定义处
  e5b79e1 fix(lz): lz_test_wpml 的 seg[0] 未初始化读取 —— 段数为 0 时显式失败
（更早已推送：）
...
- **云台俯仰必须几何反算** `[V]`（2026-09-25，用户要求）——
  ...
  否决方案：钳到相机限位 —— 会把"飞机低于目标时该往上看、
  而相机上仰能力有限，超出就是物理上做不到"伪装成做得到。改为照实返回，
  由 `LzPlan_Validate` 用 `LZ_ERR_RANGE` 拒绝。
  ⚠️ **上仰上限 2026-09-26 更正为 70°**（早期写的 30° 是照抄 M4E 的，
  DJI 官网 M4T 是 −90°~70°，固件说明佐证）。见 §2 与 CLAUDE.md。
  ⚠️ 用户同日指出：**俯仰对"目标在飞机上方/下方"是对称的** ——
  公式只回答"要让中点居中该指向哪"，不问可不可达；
  但"飞机比杆低得多"时所需的上仰角会超过 70°（12.5 m 半径、低 30 m →
  +71.6°），这是本方案**回退路径也会撞上的同一堵墙**。
...
- **设备地址有两个，别假设是哪一个** `[V]`（2026-09-26）——
  USB 扩展坞 ⇒ **`192.168.1.180`**（设备 eth0 落本机网段，本机 eth3）；
  USB 网络共享 ⇒ `192.168.42.120`（本机 eth4）。**本次 `eth4` 不存在**
  而 `.180` 通 —— 上轮交接记的 `.42.120` 已不适用。
  先 `ip -4 -br addr`，再 `ip neigh`（比 `/dev/tcp` 扫网段可靠，后者有假阳性）。
- **设备时钟落后本机 43.7 小时** `[V]`（2026-09-26 实测）——
  RTC 无电池。⇒ `tar --mtime` 必须**从设备取时间**再减 60 秒；
  按本机时间算出来的是"未来"，tar 会拒（且 make 可能报 Clock skew 并
  产出不完整的构建）。**两个方向都踩过**：未来被拒 / `@0` 让 make 静默跳过。
- **探针输出被重定向时必须 flush** `[V]`（2026-09-26 实测）——
  探针卡在 `DjiCore_Init`（飞机未通电）时，卡住**之前**那几行是回答
  "卡在哪一步"的唯一线索，而全缓冲会让它们全部丢失。
  `lz_liveview_probe.c` 已加 `say()` 包装 `vprintf` + `fflush`。
- **`CMakeCache.txt` 会记住旧开关** `[V]`（2026-09-26 踩到）——
  设备上 `build-native` 是从上一轮留的，缓存里 `LZ_BUILD_PSDK_APP=OFF`，
  于是只编出桌面测试、**没有 `bin/`**（报的是"目录不存在"，病因在缓存）。
  传新源码后要 `-DLZ_BUILD_PSDK_APP=ON -DPSDK_ROOT=$HOME/lzbuild` 显式重配。
...
**⚠️ 顺序在第三段（2026-09-26）又动了一次**：

- 用户把**设备连接方式**换成 USB 扩展坞（`192.168.1.180`），
  并说设备编译**不着急** —— 但本轮已顺手做完（设备侧能编、能跑测试）。
- **取图探针等飞机通电才能跑**（`./lz_liveview_probe scan`）。
  这是当前唯一"卡在外部条件上"的事；飞机一通电就该做，因为它是
  方案文档 §7 步骤 2 的**分母**（哪个镜头 / 通道顺序 / 分辨率）。

**用户 2026-09-25 定下的排序（仍然有效）：
...
- ~~红旗顶的实际高度~~ —— **用户 2026-09-25 已定：不用要了**，
  改为用视觉测俯仰（`VISION-GIMBAL-PITCH.md`）。
  ⚠️ **用户 2026-09-26 进一步指出：「杆高直接决定俯仰」这个说法本身是错的** ——
  无论旗在飞机上面还是下面，只要在广角画面里、在云台范围内，就该让
  **屏幕中心落在旗上**。⇒ 俯仰的目标是"画面里那个点居中"，
  几何公式只是它的一个（会受杆高误差影响的）近似。
